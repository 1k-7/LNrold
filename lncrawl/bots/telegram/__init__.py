...
class TelegramBot:
    def __init__(self):
        # Reduced max_workers for low RAM usage, especially when using Browser/Cloudscraper
        self.executor = ThreadPoolExecutor(
            max_workers=2, thread_name_prefix="telegram_bot"
        )
    ...
    def run_download(self, app, chat_id, session, context):
        try:
            ...
            # app.novel_status is checked implicitly here, as start_download 
            # sets the status to COMPLETED, FAILED, or HALTED.

            session["status"] = "Download finished"
            session["download_future"] = None
            
            if app.novel_status == "COMPLETED":
                # Only try to bind if the download successfully completed (Integrity passed)
                session["bind_future"] = self.executor.submit(
                    self.run_bind_books, app, chat_id, session, context
                )
            
        except Exception as e:
            # Check if this is the MANUAL RESTART HALT signal
            if HALT_403_SIGNAL in str(e):
                app.novel_status = "HALTED"
                error_msg = str(e).split(HALT_403_SIGNAL + ":")[-1].strip()
                session["error"] = f"A manual restart is required: {error_msg}"
                logger.critical(session["error"])
            else:
                app.novel_status = "FAILED"
                session["error"] = f"Download failed unexpectedly: {e}"
    ...
    async def check_download_progress(self, context):
        ...
        if session.get("error"):
            # Send the error message and clean up.
        ...
        if session.get("upload_pending"):
            ...
            # --- UPLOAD HALT: CHECK FINAL STATUS BEFORE STARTING UPLOAD ---
            if session["app"].novel_status != "COMPLETED":
                
                if session["app"].novel_status == "HALTED":
                    final_message = (
                        f"🛑 **PROCESS HALTED** 🛑\n"
                        f"**Reason:** 403 Ban detected.\n"
                        f"**Action:** The session has stopped. You must change your IP/VPN and run /start to try again."
                    )
                else: # FAILED status
                    final_message = (
                        f"❌ **INTEGRITY CHECK FAILED** ❌\n"
                        f"**Reason:** Not all selected chapters were downloaded successfully.\n"
                        f"**Action:** No file was uploaded. You can run /start to retry."
                    )
                
                await context.bot.send_message(chat_id, text=final_message)
            ...
            for archive in app.archived_outputs:
                # Basic file check added to prevent errors on missing files
                if not os.path.exists(archive):
                    logger.warning(f"File not found: {archive}. Skipping upload.")
                    continue
                ...
                if file_size < 49.99 * 1024 * 1024:
                    try:
                        ...
                        await context.bot.send_message(chat_id, text=f"Failed to upload {os.path.basename(archive)}")
                else:
                    await context.bot.send_message(
                        chat_id,
